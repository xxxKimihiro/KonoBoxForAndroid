...
class AutoGroupSettingsActivity :
    ProfileSettingsActivity<AutoGroupBean>(R.layout.layout_chain_settings) {

    override fun createEntity() = AutoGroupBean()

    val proxyList = ArrayList<ProxyEntity>()

    override fun AutoGroupBean.init() {
        DataStore.profileName = name
        DataStore.serverProtocol = proxies.joinToString(",")
        DataStore.autoGroupStrategy = strategy
    }

    override fun AutoGroupBean.serialize() {
        name = DataStore.profileName
        proxies = proxyList.map { it.id }
        strategy = DataStore.autoGroupStrategy
    ...
    override fun PreferenceFragmentCompat.createPreferences(
        ...
        addPreferencesFromResource(R.xml.auto_group_preferences)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar!!.setTitle(R.string.auto_group_settings)
    ...
    fun testProfileAllowed(profile: ProxyEntity): Boolean {
        ...
        if (profile.type == ProxyEntity.TYPE_AUTO_GROUP) return false
        if (profile.type == ProxyEntity.TYPE_CHAIN) return false

        for (entity in proxyList) {
            if (entity.id == profile.id) return false
        }
        return true
    }
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { (resultCode, data) ->
            if (resultCode == Activity.RESULT_OK) runOnDefaultDispatcher {
                ...
                if (!testProfileAllowed(profile)) {
                    onMainDispatcher {
                        MaterialAlertDialogBuilder(this@AutoGroupSettingsActivity).setTitle(R.string.circular_reference)
        ...
        fun bind() {
            binding.root.setOnClickListener {
                ...
                selectProfileForAdd.launch(
                    Intent(
                        this@AutoGroupSettingsActivity, ProfileSelectActivity::class.java
                    )
                )
            }
        }
